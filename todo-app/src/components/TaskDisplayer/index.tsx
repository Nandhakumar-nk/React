...
import { newTaskSubmitted } from "../../actions/taskDisplayer";
...
interface ITaskDisplayerProps {
  ...
  newTaskSubmitted: (categoryId: string, task: string) => void;
...
> {
  ...
  handleSubmit(this: any, event: React.KeyboardEvent<HTMLInputElement>) {
    if (event.keyCode === 13 && this.state.task.length > 0) {
      this.props.newTaskSubmitted(this.props.selectedCategoryId, this.state.task);
      this.setState({ task: "" });
    }
  }
...
const mapDispatchToProps = (dispatch: (arg0: any) => any) => ({
  newTaskSubmitted: (categoryId: string, task: string) =>
    dispatch(newTaskSubmitted(categoryId, task)),
